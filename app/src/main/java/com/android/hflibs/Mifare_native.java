...
public class Mifare_native {
	...
	public int InitDev()
	{
		return init_dev(DEVPATH);//0 代表初始化设备成功，-1 代表失败
	}
	...
	public byte[] SearchCard()//搜卡，处理冲突并选卡。返回选中卡片的 SN,搜卡失败，返回 null；成功返回 byte array 引用。其内容为卡片的 SN
	{
	...
	static {
		System.loadLibrary("rc663nxp-logi");
		System.loadLibrary("package-logi");
		System.loadLibrary("rc663mifare-l");
	}
}